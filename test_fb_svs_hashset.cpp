...
int main()
{
	...
	for(int i = 0; i < n_queries; ++i)
	{
		...
		std::unordered_set<size_t> fb_svs_match_pos;
		int n_matches = fb_svs(query, index, max_code_len, fb_svs_match_pos);
		std::unordered_set<size_t> str_find_match_pos;
		size_t start = 0;
		while(1)
		{
		...
		if(fb_svs_match_pos != str_find_match_pos)
		{
			...
			std::cout << "fb-svs output size - " << fb_svs_match_pos.size() << std::endl;
			std::cout << "str::find() output size - " << str_find_match_pos.size() << std::endl;
		}
	}
	return 0;
}
